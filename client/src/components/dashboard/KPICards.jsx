...
const KPICards = ({ devices = [] }) => {
    ...
    const kpiDataToUse = devices.length > 0 ? {
        ...
        avgTemperature: devices.length > 0 
            ? Math.round(devices.reduce((sum, d) => sum + (d.temperature || 0), 0) / devices.length)
            : 0,
        avgFanSpeed: devices.length > 0
            ? Math.round(devices.reduce((sum, d) => sum + (d.fanSpeed || 0), 0) / devices.length)
            : 0,
        uptime: devices.length > 0
